...
const quotes = [{ category: "flight" as const, unitPrice: 999, source: "t" }, { category: "stay" as const, unitPrice: 111, source: "t" }];
...
describe("tripStore", () => {
  ...
  it("clamps traveler/room/bed counts", () => {
    const s = createTripStore();
    s.getState().setConfig({ people: 0, rooms: -2, extraBeds: -1 });
    expect(s.getState().trip.config).toMatchObject({ people: 1, rooms: 1, extraBeds: 0 });
  });
  ...
  it("applies quotes but preserves user-edited prices", () => {
    const s = createTripStore();
    const stay = s.getState().trip.items.find((i) => i.category === "stay")!;
    s.getState().updateItem(stay.id, { unitPrice: 7 });
    s.getState().applyQuotes(quotes);
    const items = s.getState().trip.items;
    expect(items.find((i) => i.category === "flight")!.unitPrice).toBe(999);
    expect(items.find((i) => i.id === stay.id)).toMatchObject({ unitPrice: 7, overridden: true });
  });
  ...
  it("adds and removes items", () => {
    ...
    expect(s.getState().trip.items).toHaveLength(7);
  });
